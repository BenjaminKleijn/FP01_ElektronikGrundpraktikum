...
\begin{flalign*}
  ...
  &\hspace*{1.5cm} R_C = \frac{U_V}{2 \cdot I_C \cdot \left(1 + \frac{1}{V_U}\right)} &&&& = \qty{6.80}{\kilo\ohm} \quad (\qty{7.14}{\kilo\ohm}) \tag{B.RC}\label{eq:R_C} \\
  ...
  &\hspace*{1.5cm} f_{ug}=(2\pi\cdot R_{\text{EIN}} \cdot C_{\text{EIN}})^{-1} &&&& =\qty{37.40}{\hertz} \tag{B.1}
  \label{eq:f_ug_theo_emitter}
\end{flalign*}
...
\begin{flalign*}
  ...
  \intertext{erneutes Berechnen der unteren Grenzfrequenz mit neu gewählter Kapazität:}
  &\hspace*{1.5cm} f_{ug}=(2\pi\cdot R_{\text{EIN}}\cdot C_{\text{EIN}})^{-1}&&&&=\qty{49.10}{\hertz} \tag{B.2} \label{eq:f_ug_theo_kollektor}
\end{flalign*}
...
\includepdf[pages=-,addtotoc={1, section, 1, Plot Code,sec:python_plots}]{../Plots.pdf}

\includepdf[pages=-,addtotoc={1, section, 1, Tabellen Code,sec:python_tabelle}]{../Tabellen.pdf}

\includepdf[pages=-,addtotoc={1, section, 1, Dimensionierung Code,sec:python_Dimensionierung}]{../Dimensionierung.pdf}
